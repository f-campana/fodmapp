...
- A local `pre-push` Git hook is installed via `.githooks` and runs `./.github/scripts/quality-gate.sh --full` automatically on `git push` (requires `pnpm hook:install`/`pnpm prepare`, which runs during install). Delete-only pushes (for example `git push --delete`) are skipped.
...
- Keep `/Users/fabiencampana/Documents/fodmapp` (main worktree) clean at all times.
