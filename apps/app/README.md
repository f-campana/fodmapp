...
- The API CORS allowlist follows `CLERK_AUTHORIZED_PARTIES` for runtime Clerk flows. Production must include `https://app.fodmapp.fr` there; add `API_CORS_ALLOW_ORIGINS` only for extra origins that are not part of the Clerk authorized-party list.
- If a production deployment is missing any Clerk runtime key, `/espace`, `/espace/suivi`, `/sign-in`, and `/sign-up` now surface an explicit deployment-configuration error instead of silently behaving like a local preview build.
