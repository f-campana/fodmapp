...
- Resolved: desktop copy actions are keyboard reachable without relying on hover-only reveal.
- Resolved: jump-link scrolling now ignores stale RAF loops and honors `prefers-reduced-motion`.
- Mitigated: mount-time scroll reset is gated to avoid overriding hash and back/forward restoration.
- Expanded coverage: `color.stories.tsx` play assertions now cover keyboard tab reachability for copy controls and rapid sequential jump-link behavior.
