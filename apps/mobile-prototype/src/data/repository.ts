import { defaultPreferences, foods, type Food, type Preferences } from './mockData';
...
export async function listFoods(query = ''): Promise<Food[]> {
  ...
  return foods.filter((food) => [food.name, food.category, ...food.tags].join(' ').toLowerCase().includes(normalized));
}
...
export async function getDashboardSnapshot() {
  ...
  return {
    ...
    availableSwaps: foods.reduce((count, food) => count + food.alternatives.length, 0)
  };
}
