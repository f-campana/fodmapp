...
void test("listFoods returns all foods for empty query", async () => {
  const foods = await listFoods("");
  assert.ok(foods.length >= 3);
});

void test("listFoods filters by tag/category/name", async () => {
...
void test("getFoodById returns undefined for unknown food", async () => {
  const food = await getFoodById("unknown-food");
  assert.equal(food, undefined);
});

void test("dashboard snapshot counts are coherent", async () => {
