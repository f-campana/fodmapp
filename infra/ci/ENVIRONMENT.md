...
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------- | ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `API_DB_URL`                        | `api/app/config.py`, `api/tests/conftest.py`, `.github/workflows/api.yml`, `scripts/dbmate.sh`, `api/scripts/phase3_bootstrap.py`, `api/scripts/phase3_promote.py`, `package.json`               | yes                           | `postgresql://.../fodmap_test`        | Primary FastAPI DB connection string, dbmate target DSN, initial persistent bootstrap target, and persistent Phase 3 promote target. Local disposable Postgres URLs should set `sslmode=disable`; hosted Neon URLs keep provider SSL settings. |
| `API_NAME`                          | `api/app/config.py`                                                                                                                                                                              | no                            | `fodmapp-api`                         | Health metadata.                                                                                                                                                                                                                               |
| `API_VERSION`                       | `api/app/config.py`                                                                                                                                                                              | no                            | `v0`                                  | Health metadata.                                                                                                                                                                                                                               |
| `API_DEBUG_CLERK_CLAIMS`            | `api/app/auth.py`                                                                                                                                                                                | no                            | `false`                               | Non-production, explicitly gated safe claim-shape logging for Clerk bearer debugging. Logs claim keys plus selected claim values (`iss`, `sub`, `azp`, `aud`) without logging the raw token.                                                   |
| `PRODUCTS_FEATURE_ENABLED`          | `api/app/config.py`, `api/app/routers/products.py`                                                                                                                                               | no                            | `false`                               | Enables the guided `/v0/products` lane at runtime. When disabled, product endpoints return `404 not_found`.                                                                                                                                    |
| `PRODUCTS_STALE_AFTER_HOURS`        | `api/app/config.py`, `etl/products/runner.py`, `etl/products/compiler.py`                                                                                                                        | no                            | `72`                                  | Freshness horizon for Open Food Facts product snapshots before public reads enqueue a refresh request.                                                                                                                                         |
| `PRODUCTS_REFRESH_COOLDOWN_SECONDS` | `api/app/config.py`, `api/app/products_service.py`, `etl/products/runner.py`                                                                                                                     | no                            | `900`                                 | Minimum cooldown before a public read can re-queue the same product refresh request.                                                                                                                                                           |
| `OFF_API_BASE_URL`                  | `api/app/config.py`, `etl/products/off_client.py`, `etl/products/runner.py`                                                                                                                      | no                            | `https://world.openfoodfacts.org`     | Open Food Facts API origin for the guided packaged-product lane.                                                                                                                                                                               |
| `OFF_USER_AGENT`                    | `api/app/config.py`, `etl/products/off_client.py`, `etl/products/runner.py`                                                                                                                      | no                            | `FODMAPP/0.1 (contact@fodmapp.local)` | Required user-agent identifier for friendly Open Food Facts access.                                                                                                                                                                            |
| `OFF_TIMEOUT_SECONDS`               | `api/app/config.py`, `etl/products/off_client.py`, `etl/products/runner.py`                                                                                                                      | no                            | `3`                                   | HTTP timeout for Open Food Facts requests in the guided product compiler.                                                                                                                                                                      |
...
| `PSQL_BIN`                          | `etl/phase2/scripts/phase2_replay_from_zero.sh`, `etl/phase3/scripts/phase3_seed_for_api_ci.sh`, `api/scripts/phase3_bootstrap.py`, `api/scripts/phase3_promote.py`, `.github/workflows/api.yml` | no                            | `psql` / auto-detected                | Path to `psql` executable for replay, bootstrap, seeded Phase 3 CI, and manual Phase 3 promote runs.                                                                                                                                           |
| `CIQUAL_DATA_DIR`                   | `etl/phase2/scripts/phase2_replay_from_zero.sh`, `api/scripts/phase3_bootstrap.py`, `etl/ciqual/fetch_ciqual_data.sh`                                                                            | no                            | script default path                   | Override location of CIQUAL files.                                                                                                                                                                                                             |
| `CIQUAL_XLSX`                       | `etl/phase2/scripts/phase2_replay_from_zero.sh`, `api/scripts/phase3_bootstrap.py`, `.github/workflows/api.yml`                                                                                  | no                            | exported by fetch script in CI        | CIQUAL XLSX override for replay/bootstrap.                                                                                                                                                                                                     |
| `CIQUAL_ALIM_XML`                   | `etl/phase2/scripts/phase2_replay_from_zero.sh`, `api/scripts/phase3_bootstrap.py`, `.github/workflows/api.yml`                                                                                  | no                            | exported by fetch script in CI        | CIQUAL `alim` XML override for replay/bootstrap.                                                                                                                                                                                               |
| `CIQUAL_GRP_XML`                    | `etl/phase2/scripts/phase2_replay_from_zero.sh`, `api/scripts/phase3_bootstrap.py`, `.github/workflows/api.yml`                                                                                  | no                            | exported by fetch script in CI        | CIQUAL `alim_grp` XML override for replay/bootstrap.                                                                                                                                                                                           |
...
## Guided Product Lane Runtime Notes

- `/v0/products/*` is a guided/discovery contract tier, separate from canonical food rollups, swaps, and safe-harbors.
- Public product reads may enqueue one bounded refresh request when compiled state is missing or stale, but they do not call Open Food Facts inline.
- The Open Food Facts client must use the configured `OFF_USER_AGENT` and remain within normal low-volume compile/refresh usage.
...
Initial persistent load for a long-lived Neon branch now reuses the same DB/runtime variables.

Contract:

- `pnpm phase3:bootstrap:check`, `pnpm phase3:bootstrap`, and `pnpm phase3:bootstrap:status` reuse `API_DB_URL`
- the runner uses `PSQL_BIN` for SQL execution and `CIQUAL_XLSX` / `CIQUAL_ALIM_XML` / `CIQUAL_GRP_XML` (or `CIQUAL_DATA_DIR` defaults) for explicit CIQUAL input files
- `phase3:bootstrap:check` requires a dbmate-migrated schema-only database and fails clearly when CIQUAL files are missing, when Phase 2 / Phase 3 / publish state already exists, or when the target is only partially loaded
- `phase3:bootstrap` is non-destructive: it does not drop or recreate the database, does not mutate tracked review CSVs, and does not depend on git restore
- the runner synthesizes a temporary reviewed packet from the committed MVP review decisions and the live Gate A export, then runs activation apply/checks against that temporary packet
- after bootstrap completes, the steady-state operator path becomes `pnpm phase3:promote`

Semantics:

- scope is intentionally schema-only-only for first-time persistent load; partially loaded databases are out of scope
- the reusable temp review-packet mechanism is also used by `etl/phase3/scripts/phase3_seed_for_api_ci.sh`, so seeded CI no longer restores tracked review CSVs from git
- `.github/workflows/api.yml` adds `phase3-bootstrap-smoke` to prove bootstrap succeeds on a fresh dbmate-only database, then hands off cleanly to `phase3:promote`
...
| `NEXT_PUBLIC_API_BASE_URL`              | Public app reads via `@fodmapp/api-client` + `/me/*` API access | no          | Public read client returns `api_not_configured`; consent bootstrap stays disabled until configured. Use the API origin; app clients normalize `/v0`.                                  |
| `EXPO_PUBLIC_API_BASE_URL`              | Expo mobile prototype public catalog reads                      | no          | Connected catalog screens return handled config/network errors when missing. Set it in `apps/mobile-prototype/.env.local`; use the API origin and let the client normalize `/v0`.     |
| `EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY`     | Expo mobile prototype Clerk auth shell                          | no          | Mobile auth shell stays signed out with a handled configuration message when missing. Set it in `apps/mobile-prototype/.env.local`; the session token cache uses `expo-secure-store`. |
