...
| `BARCODE_FEATURE_ENABLED`    | `api/app/config.py`, barcode public router                                                                                   | no                            | `false`                               | Enables public barcode lookup (`/v0/barcodes/{barcode}`). |
| `BARCODE_INTERNAL_ENABLED`   | `api/app/config.py`, barcode internal auth/router                                                                            | no                            | `false`                               | Enables internal barcode curation routes (`/v0/internal/barcodes/*`). |
| `BARCODE_ADMIN_TOKEN`        | `api/app/config.py`, `api/app/auth.py`                                                                                       | when `BARCODE_INTERNAL_ENABLED=true` | empty                        | Bearer token required by internal barcode routes.   |
| `OFF_API_BASE_URL`           | `api/app/config.py`, `api/app/providers/open_food_facts.py`                                                                  | no                            | `https://world.openfoodfacts.org`     | Open Food Facts base URL used for barcode enrichment. |
| `OFF_USER_AGENT`             | `api/app/config.py`, `api/app/providers/open_food_facts.py`                                                                  | yes for OFF calls             | `FODMAPPlatform/0.1 (contact@example.com)` | Custom `User-Agent` header for OFF requests.    |
| `OFF_TIMEOUT_SECONDS`        | `api/app/config.py`, `api/app/providers/open_food_facts.py`                                                                  | no                            | `3`                                   | Timeout in seconds for OFF HTTP requests.           |
| `BARCODE_CACHE_TTL_HOURS`    | `api/app/config.py`, barcode service                                                                                         | no                            | `168`                                 | Cache TTL in hours for OFF payloads.                |
| `BARCODE_HEURISTIC_THRESHOLD`| `api/app/config.py`, barcode service                                                                                         | no                            | `0.75`                                | Minimum score to persist a heuristic link.          |
| `BARCODE_HEURISTIC_MARGIN`   | `api/app/config.py`, barcode service                                                                                         | no                            | `0.10`                                | Minimum lead over next candidate to persist heuristic link. |
