...
DO $$
BEGIN
  IF to_regclass('public.v_phase3_rollup_subtype_levels_latest') IS NULL THEN
    EXECUTE $view$
...
DO $$
BEGIN
  IF to_regclass('public.v_phase3_rollups_latest_full') IS NULL THEN
    EXECUTE $view$
...
BEGIN
  ...
  AND NOT EXISTS (
    ...
    backfill_publish_id := gen_random_uuid();
    ...
    SELECT
      ...
      'dbmate migration backfill from existing Phase 3 state';
