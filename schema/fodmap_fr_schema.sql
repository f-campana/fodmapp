...
CREATE TABLE product_food_links (
  ...
  food_id UUID NOT NULL REFERENCES foods (food_id) ON DELETE CASCADE,
  link_method TEXT NOT NULL CHECK (link_method IN ('exact_name', 'manual', 'nlp', 'heuristic')),
...
CREATE TABLE product_provider_snapshots (
...
CREATE TABLE product_food_candidates (
...
CREATE TABLE product_assessment_subtypes (
...
CREATE INDEX idx_product_provider_snapshots_lookup
  ON product_provider_snapshots (normalized_code, fetched_at DESC);
CREATE INDEX idx_product_provider_snapshots_product
  ON product_provider_snapshots (product_id, fetched_at DESC);
CREATE INDEX idx_product_refresh_requests_status_refresh_after
  ON product_refresh_requests (status, refresh_after, last_requested_at DESC);
CREATE INDEX idx_product_ingredients_normalized_name
  ON product_ingredients (normalized_name);
CREATE INDEX idx_product_food_candidates_lookup
  ON product_food_candidates (product_id, line_no, candidate_rank);
CREATE UNIQUE INDEX uq_product_food_candidates_selected
  ON product_food_candidates (product_id, line_no)
  WHERE is_selected;
CREATE INDEX idx_product_assessments_product
  ON product_assessments (product_id, computed_at DESC);
CREATE INDEX idx_product_assessment_subtypes_source_food
  ON product_assessment_subtypes (source_food_id, subtype_code);
CREATE INDEX idx_product_review_events_product
