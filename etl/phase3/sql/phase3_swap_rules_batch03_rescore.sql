...
BEGIN
  ...
  IF row_count < 1 OR row_count > 40 THEN
    RAISE EXCEPTION 'Batch03 generated file must contain 1..40 rows, got %', row_count;
  END IF;

  SELECT COUNT(*) INTO bad_count
  FROM stg_generated
  WHERE notes <> 'phase3_batch03_rule'
     OR valid_from <> DATE '2026-02-21';
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'Batch03 generated file contains non-batch03 scope rows';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'rank2 exclusion failed in batch03 generated scope';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'duplicate rule_key values in batch03 generated scope';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'batch03 DB scope missing one or more generated rules';
  END IF;
END $$;
...
WITH updated AS (
  ...
  SET
    scoring_version = 'v2_full_rollup_2026_02_21_batch03',
    fodmap_safety_score = rs.recomputed_fodmap_safety_score
  FROM rescore_snapshot rs
  WHERE s.swap_rule_id = rs.swap_rule_id
    AND (
      s.scoring_version IS DISTINCT FROM 'v2_full_rollup_2026_02_21_batch03'
...
SELECT
  ...
  rs.to_priority_rank,
  'v2_full_rollup_2026_02_21_batch03'::TEXT AS scoring_version_snapshot,
...
\copy review_export TO 'etl/phase3/decisions/phase3_swap_batch03_review_v1.csv' WITH (FORMAT csv, HEADER true)
