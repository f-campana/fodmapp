...
BEGIN
  ...
  IF matrix_count <> 35 THEN
    RAISE EXCEPTION 'coverage matrix must contain 35 missing subtype rows, got %', matrix_count;
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'coverage batchB contains unsupported method/source combinations';
  END IF;
  ...
  IF db_count <> csv_count THEN
    RAISE EXCEPTION 'current DB rows from coverage_batchB_v1 (%) must equal CSV count (%)', db_count, csv_count;
  END IF;
  ...
  WHERE priority_rank IN (9, 16, 24, 25, 26, 29, 32)
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'coverage uplift expectation failed: all Batch B targets must reach known_subtypes_count >= 2';
  END IF;

  -- Global coverage guard for this sprint baseline.
  SELECT COUNT(*) INTO one_of_six_bucket
  FROM v_phase3_rollups_latest_full
  WHERE known_subtypes_count = 1;

  IF one_of_six_bucket > 8 THEN
    RAISE EXCEPTION 'global 1/6 bucket regression: expected <=8 after Batch B lactose pass, got %', one_of_six_bucket;
  END IF;
...
WHERE v.priority_rank IN (9, 16, 24, 25, 26, 29, 32)
