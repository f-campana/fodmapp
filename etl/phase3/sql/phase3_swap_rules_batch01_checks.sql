...
BEGIN
  ...
  IF generated_count < 1 OR generated_count > 40 THEN
    RAISE EXCEPTION 'Batch01 generated row count must be within 1..40, got %', generated_count;
  END IF;

  SELECT COUNT(*) INTO review_count FROM stg_review;
  IF review_count <> generated_count THEN
    RAISE EXCEPTION 'Batch01 review row count (%) must equal generated row count (%)', review_count, generated_count;
  END IF;

  SELECT COUNT(*) INTO db_count FROM batch_rules;
  IF db_count <> generated_count THEN
    RAISE EXCEPTION 'Batch01 DB scoped row count (%) must equal generated row count (%)', db_count, generated_count;
  END IF;
  ...
  JOIN swap_rules r
   ...
   AND COALESCE(r.notes, '') <> 'phase3_batch01_rule';
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'generated scope collides with open-validity non-batch01 rules in % rows', bad_count;
  END IF;
  ...
  SELECT COUNT(*) INTO bad_count
  FROM batch_rules
  WHERE scoring_version <> 'v2_full_rollup_2026_02_19_batch01';
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'batch01 scoring_version mismatch in % rows', bad_count;
  END IF;
  ...
  IF active_count + draft_count <> generated_count THEN
    RAISE EXCEPTION 'status accounting mismatch for batch01 rules';
  END IF;
END $$;
