...
-- Phase 3.6 Batch C coverage uplift apply.
-- Scope: remaining 8 foods at 1/6 coverage.
...
BEGIN
  ...
  IF bad_count < 1 THEN
    RAISE EXCEPTION 'coverage batchC staging file is empty';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'coverage batchC contains rows outside locked rank scope';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'coverage batchC contains invalid subtype codes';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'coverage batchC contains unsupported method values (allowed: derived_from_nutrient, literature, expert_estimate)';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'coverage batchC contains unsupported source_slug values';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'coverage batchC confidence scores must be in [0,1]';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'coverage batchC slug/rank mismatch detected';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'coverage batchC duplicate rank/subtype rows detected';
  END IF;
  ...
  WHERE notes IS NULL
     OR notes = ''
     OR notes NOT LIKE 'coverage_batchC_v1:%';
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'coverage batchC rows must use coverage_batchC_v1 notes markers';
  END IF;
  ...
  WHERE amount_g_per_100g IS NULL
     ...
     OR citation_ref = '';
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'coverage batchC rows must include amount + serving + citation fields';
  END IF;
  ...
  -- Garlic-infused oil special policy: near-zero inferred non-lactose rows.
  SELECT COUNT(*) INTO bad_count
  FROM stg_measurements
  WHERE notes LIKE 'coverage_batchC_v1:garlic_oil_fodmap_zero_inference%'
    AND (
      priority_rank <> 3
      OR subtype_code NOT IN ('fructose', 'gos', 'mannitol', 'sorbitol')
      OR source_slug <> 'monash_app_v4_reference'
      OR method NOT IN ('expert_estimate', 'derived_from_nutrient')
      OR evidence_tier <> 'inferred'
      OR confidence_score < 0.90
      OR comparator NOT IN ('eq', 'lt', 'lte')
      OR amount_g_per_100g > 0.1
    );
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'garlic_oil_fodmap_zero_inference rows violate rank3 near-zero policy';
  END IF;
END $$;

-- Idempotency: replace only prior batchC injected rows.
...
WHERE m.food_id = p.resolved_food_id
  ...
  AND p.priority_rank IN (3, 6, 8, 10, 13, 17, 27, 28)
  AND m.source_id IN (
    SELECT source_id
    FROM sources
  ...
  AND m.notes LIKE 'coverage_batchC_v1:%';
...
SELECT
  ...
  src.source_id,
  format('coverage_batchC_v1:%s:%s', s.priority_rank, s.subtype_code),
  s.amount_raw,
  s.comparator,
  s.amount_g_per_100g,
