...
BEGIN
  ...
  IF generated_count < 1 OR generated_count > 40 THEN
    RAISE EXCEPTION 'Batch03 generated row count must be within 1..40, got %', generated_count;
  END IF;

  SELECT COUNT(*) INTO review_count FROM stg_review;
  IF review_count <> generated_count THEN
    RAISE EXCEPTION 'Batch03 review row count (%) must equal generated row count (%)', review_count, generated_count;
  END IF;

  SELECT COUNT(*) INTO db_count FROM batch_rules;
  IF db_count <> generated_count THEN
    RAISE EXCEPTION 'Batch03 DB scoped row count (%) must equal generated row count (%)', db_count, generated_count;
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'generated scope collides with exact open-validity non-batch03 rules in % rows', bad_count;
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'generated scope collides with reverse open-validity non-batch03 rules in % rows', bad_count;
  END IF;
  ...
  SELECT COUNT(*) INTO bad_count
  FROM batch_rules
  WHERE scoring_version <> 'v2_full_rollup_2026_02_21_batch03';
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'batch03 scoring_version mismatch in % rows', bad_count;
  END IF;
  ...
  IF active_count + draft_count <> generated_count THEN
    RAISE EXCEPTION 'status accounting mismatch for batch03 rules';
  END IF;
END $$;
