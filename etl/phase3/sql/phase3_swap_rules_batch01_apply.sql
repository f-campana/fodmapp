...
-- Phase 3.3 Batch01: apply generated draft swap rules.
...
BEGIN
  ...
  IF row_count < 1 OR row_count > 40 THEN
    RAISE EXCEPTION 'Batch01 candidate count must be within 1..40, got %', row_count;
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'Batch01 rows contain ranks outside 1..42';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'Batch01 rows contain self-pairs';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'Batch01 rank2 exclusion failed';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'Batch01 duplicate from/to/rule_kind rows detected';
  END IF;
  ...
  WHERE status <> 'draft';
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'Batch01 status must be draft for all rows';
  END IF;

  SELECT COUNT(*) INTO bad_count
  FROM stg_rules
  WHERE notes <> 'phase3_batch01_rule';
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'Batch01 notes must be phase3_batch01_rule';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'Batch01 valid_from must be locked to 2026-02-19';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'Batch01 contains invalid rule_kind';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'Batch01 contains invalid evidence_tier';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'Batch01 instructions must be populated in both FR and EN';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'Batch01 ratio constraints invalid';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'Batch01 score values must be within [0,1]';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'Batch01 from/to ranks must resolve and target rows must be threshold_set';
  END IF;
END $$;
