...
BEGIN
  ...
  IF row_count < 1 OR row_count > 40 THEN
    RAISE EXCEPTION 'Batch01 generated file must contain 1..40 rows, got %', row_count;
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'Batch01 generated file contains non-batch01 scope rows';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'rank2 exclusion failed in batch01 generated scope';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'duplicate rule_key values in batch01 generated scope';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'batch01 DB scope missing one or more generated rules';
  END IF;
END $$;
...
WITH updated AS (
  ...
  SET
    scoring_version = 'v2_full_rollup_2026_02_19_batch01',
    fodmap_safety_score = rs.recomputed_fodmap_safety_score
  FROM rescore_snapshot rs
  WHERE s.swap_rule_id = rs.swap_rule_id
    AND (
      s.scoring_version IS DISTINCT FROM 'v2_full_rollup_2026_02_19_batch01'
...
SELECT
  ...
  rs.to_priority_rank,
  'v2_full_rollup_2026_02_19_batch01'::TEXT AS scoring_version_snapshot,
...
\copy review_export TO '/Users/fabiencampana/Documents/Fodmap/etl/phase3/decisions/phase3_swap_batch01_review_v1.csv' WITH (FORMAT csv, HEADER true)
