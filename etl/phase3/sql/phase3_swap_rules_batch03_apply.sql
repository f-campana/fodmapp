...
-- Phase 3.7 Batch03: apply generated draft swap rules.
...
BEGIN
  ...
  IF row_count < 1 OR row_count > 40 THEN
    RAISE EXCEPTION 'Batch03 candidate count must be within 1..40, got %', row_count;
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'Batch03 rows contain ranks outside 1..42';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'Batch03 rows contain self-pairs';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'Batch03 rank2 exclusion failed';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'Batch03 duplicate from/to/rule_kind rows detected';
  END IF;
  ...
  WHERE status <> 'draft';
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'Batch03 status must be draft for all rows';
  END IF;

  SELECT COUNT(*) INTO bad_count
  FROM stg_rules
  WHERE notes <> 'phase3_batch03_rule';
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'Batch03 notes must be phase3_batch03_rule';
  END IF;

  SELECT COUNT(*) INTO bad_count
  FROM stg_rules
  WHERE valid_from <> DATE '2026-02-21';
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'Batch03 valid_from must be locked to 2026-02-21';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'Batch03 rule_kind must be direct_swap for all rows';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'Batch03 contains invalid evidence_tier';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'Batch03 instructions must be populated in both FR and EN';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'Batch03 ratio constraints invalid';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'Batch03 score values must be within [0,1]';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'Batch03 same-subtype direct_swap gate failed in staged rows';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'Batch03 from/to ranks must resolve and target rows must be threshold_set';
  END IF;
END $$;
