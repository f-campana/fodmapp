...
-- Phase 3.4 Batch02 candidate generation (42-food universe).
...
CREATE TEMP TABLE generated_batch02 ON COMMIT DROP AS
...
SELECT
  FORMAT('B02_%s', LPAD(so.row_num::TEXT, 3, '0')) AS rule_key,
  ...
  'phase3_batch02_rule'::TEXT AS notes,
...
BEGIN
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'exact open-validity non-batch02 pair collision detected on % generated rows', bad_count;
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'reverse open-validity non-batch02 pair collision detected on % generated rows', bad_count;
  END IF;
END $$;
