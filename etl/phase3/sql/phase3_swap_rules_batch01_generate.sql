...
-- Phase 3.3 Batch01 candidate generation (42-food universe).
...
CREATE TEMP TABLE generated_batch01 ON COMMIT DROP AS
...
SELECT
  FORMAT('B01_%s', LPAD(so.row_num::TEXT, 3, '0')) AS rule_key,
  ...
  DATE '2026-02-19' AS valid_from,
  'draft'::TEXT AS status,
  'phase3_batch01_rule'::TEXT AS notes,
...
BEGIN
  ...
  JOIN swap_rules r
   ...
   AND COALESCE(r.notes, '') <> 'phase3_batch01_rule';
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'existing open-validity non-batch01 pair collision detected on % generated rows', bad_count;
  END IF;
END $$;

\copy generated_batch01 TO '/Users/fabiencampana/Documents/Fodmap/etl/phase3/data/phase3_swap_rules_batch01_generated_v1.csv' WITH (FORMAT csv, HEADER true)
