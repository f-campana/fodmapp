...
DECLARE
  ...
  rank10_non_lactose_found INTEGER;
  rank13_non_lactose_found INTEGER;
  rank3_blocked_missing INTEGER;
BEGIN
  SELECT COUNT(*) INTO matrix_count FROM stg_matrix;
  IF matrix_count <> 40 THEN
    RAISE EXCEPTION 'coverage matrix must contain 40 missing subtype rows, got %', matrix_count;
  END IF;

  SELECT COUNT(*) INTO bad_count
  FROM stg_matrix
  WHERE priority_rank NOT IN (3, 6, 8, 10, 13, 17, 27, 28)
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'generic blocked reason insufficient_variant_specific_evidence is forbidden in batchC';
  END IF;
  ...
  IF non_lactose_found < 8 THEN
    RAISE EXCEPTION 'batchC research-yield floor failed: non-lactose found rows must be >= 8, got %', non_lactose_found;
  END IF;

  SELECT COUNT(*) INTO rank10_non_lactose_found
  FROM stg_matrix
  WHERE priority_rank = 10
    AND measurement_found = TRUE
    AND subtype_code <> 'lactose';

  IF rank10_non_lactose_found < 1 THEN
    RAISE EXCEPTION 'targeted proof failed: rank 10 must gain at least one non-lactose found subtype';
  END IF;

  SELECT COUNT(*) INTO rank13_non_lactose_found
  FROM stg_matrix
  WHERE priority_rank = 13
    AND measurement_found = TRUE
    AND subtype_code <> 'lactose';

  IF rank13_non_lactose_found < 1 THEN
    RAISE EXCEPTION 'targeted proof failed: rank 13 must gain at least one non-lactose found subtype';
  END IF;

  -- Rank 3 special contract: no blocked missing subtype cells.
  SELECT COUNT(*) INTO rank3_blocked_missing
  FROM stg_matrix
  WHERE priority_rank = 3
    AND measurement_found = FALSE;
  IF rank3_blocked_missing > 0 THEN
    RAISE EXCEPTION 'rank 3 special-case failed: garlic-infused oil cannot retain blocked missing subtype cells';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'coverage batchC contains unsupported method/source combinations';
  END IF;

  SELECT COUNT(*) INTO bad_count
  FROM stg_measurements
  WHERE notes LIKE 'coverage_batchC_v1:plant_lactose_zero_inference%'
    AND (
  ...
  SELECT COUNT(*) INTO bad_count
  FROM stg_measurements
  WHERE notes LIKE 'coverage_batchC_v1:polyols_proxy_%'
    AND (
  ...
  SELECT COUNT(*) INTO bad_count
  FROM stg_measurements
  WHERE notes LIKE 'coverage_batchC_v1:excess_from_ciqual%'
    AND (
  ...
  WHERE notes LIKE 'coverage_batchC_v1:%'
    AND is_current = TRUE;
  IF db_count <> csv_count THEN
    RAISE EXCEPTION 'current DB rows from coverage_batchC_v1 (%) must equal CSV count (%)', db_count, csv_count;
  END IF;
  ...
  WHERE priority_rank IN (3, 6, 8, 10, 13, 17, 27, 28)
    AND known_subtypes_count < 1;
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'coverage regression: found target rows with known_subtypes_count < 1';
  END IF;

  -- Plant lactose inference baseline: all target foods should now be at least 2/6.
  SELECT COUNT(*) INTO bad_count
  FROM v_phase3_rollups_latest_full
  WHERE priority_rank IN (3, 6, 8, 10, 13, 17, 27, 28)
    AND known_subtypes_count < 2;
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'coverage uplift expectation failed: all Batch C targets must reach known_subtypes_count >= 2';
  END IF;

  -- Global coverage guard for this sprint baseline: eliminate remaining 1/6 rows.
  SELECT COUNT(*) INTO one_of_six_bucket
  FROM v_phase3_rollups_latest_full
  WHERE known_subtypes_count = 1;

  IF one_of_six_bucket <> 0 THEN
    RAISE EXCEPTION 'global 1/6 bucket guard failed: expected 0 after Batch C, got %', one_of_six_bucket;
  END IF;

  -- Batch quality diagnostic: should not worsen vs current baseline (12 rows).
  ...
  WHERE r.notes IN ('phase3_batch01_rule', 'phase3_batch02_rule', 'phase3_batch03_rule')
    AND vt.coverage_ratio < 0.50;

  IF low_coverage_targets > 12 THEN
    RAISE EXCEPTION 'coverage impact regression: low-coverage target count increased above baseline 12 (got %)', low_coverage_targets;
  END IF;
END $$;
...
WHERE v.priority_rank IN (3, 6, 8, 10, 13, 17, 27, 28)
