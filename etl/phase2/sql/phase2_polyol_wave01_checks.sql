...
-- Phase 2.7 Polyol Wave 1 hard checks
...
WHERE p.priority_rank IN (30,31,32,33,35,36)
...
BEGIN
  ...
  IF v_resolved_links <> 39 THEN
    RAISE EXCEPTION 'resolved links check failed: expected 39, got %', v_resolved_links;
  END IF;

  IF v_unresolved_rows <> 3 THEN
    RAISE EXCEPTION 'unresolved check failed: expected 3, got %', v_unresolved_rows;
  END IF;
  ...
  FROM phase2_priority_foods
  WHERE priority_rank IN (30,31,32,33,35,36);
  ...
  IF COALESCE(v_polyol_priority, -1) <> 7 THEN
    RAISE EXCEPTION 'polyol sorbitol priority check failed: expected 7, got %', COALESCE(v_polyol_priority, -1);
  END IF;

  IF COALESCE(v_polyol_resolved, -1) <> 7 THEN
    RAISE EXCEPTION 'polyol sorbitol resolved check failed: expected 7, got %', COALESCE(v_polyol_resolved, -1);
  END IF;

  IF COALESCE(v_polyol_completed, -1) <> 7 THEN
    RAISE EXCEPTION 'polyol sorbitol completed check failed: expected 7, got %', COALESCE(v_polyol_completed, -1);
  END IF;

  IF COALESCE(v_polyol_unresolved, -1) <> 0 THEN
    RAISE EXCEPTION 'polyol sorbitol unresolved check failed: expected 0, got %', COALESCE(v_polyol_unresolved, -1);
  END IF;

  IF COALESCE(v_polyol_pending, -1) <> 0 THEN
    RAISE EXCEPTION 'polyol sorbitol pending check failed: expected 0, got %', COALESCE(v_polyol_pending, -1);
  END IF;
  ...
  IF v_with_candidates <> 1 THEN
    RAISE EXCEPTION 'candidate pool check failed: expected 1 unresolved-with-candidates, got %', v_with_candidates;
  END IF;

  IF v_without_candidates <> 2 THEN
    RAISE EXCEPTION 'candidate pool check failed: expected 2 unresolved-without-candidates, got %', v_without_candidates;
  END IF;
  ...
  FROM (
    ...
    WHERE ffm.source_record_ref IN (
      'yao2005_polyols_apple_raw_proxy',
      'yao2005_polyols_pear_raw_proxy',
      'yao2005_polyols_sweet_cherry_raw_proxy',
      'yao2005_polyols_peach_raw_proxy',
      'yao2005_polyols_plum_raw_proxy',
      'yao2005_polyols_prune_dried_proxy'
    )
    GROUP BY
      ffm.food_id,
      ffm.fodmap_subtype_id,
...
SELECT
  'PASS' AS phase2_polyol_wave01_checks,
  COUNT(*) FILTER (WHERE priority_rank IN (30,31,32,33,35,36) AND status = 'threshold_set') AS wave_threshold_set_rows,
