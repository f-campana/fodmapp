...
-- Phase 2.6 GOS Wave 2 execution script (mutating, idempotent)
...
\copy wave_decisions_stg (priority_rank,food_label,variant_label,decision,resolution_method,candidate_food_id,candidate_ciqual_code,new_food_slug,new_food_name_fr,new_food_name_en,new_food_preparation_state,resolution_notes,reviewed_by,reviewed_at) FROM 'etl/phase2/decisions/phase2_gos_wave02_decisions.csv' WITH (FORMAT csv, HEADER true)
\copy wave_new_foods_stg (priority_rank,new_food_slug,new_food_name_fr,new_food_name_en,new_food_preparation_state,status,custom_ref_value,category_code,notes) FROM 'etl/phase2/data/phase2_gos_wave02_new_foods.csv' WITH (FORMAT csv, HEADER true)
\copy wave_measurements_stg (priority_rank,food_id,fodmap_subtype,amount_per_100g,comparator,serving_g,amount_per_serving,source_slug,citation_ref,evidence_tier,confidence_score,observed_at,method,notes) FROM 'etl/phase2/data/phase2_gos_wave02_measurements.csv' WITH (FORMAT csv, HEADER true)
\copy wave_thresholds_stg (priority_rank,food_id,fodmap_subtype,serving_g,low_max_g,moderate_max_g,source_slug,citation_ref,evidence_tier,confidence_score,valid_from,notes) FROM 'etl/phase2/data/phase2_gos_wave02_thresholds.csv' WITH (FORMAT csv, HEADER true)

DO $$
DECLARE
  expected_ranks INTEGER[] := ARRAY[24,25,26,27,28,29];
  got_ranks INTEGER[];
  bad_count INTEGER;
BEGIN
  ...
  SELECT COUNT(*) INTO bad_count
  FROM wave_decisions_stg
  WHERE (priority_rank = 25 AND (decision <> 'resolve_existing' OR resolution_method NOT IN ('name_match', 'slug_match', 'manual')))
     OR (priority_rank <> 25 AND (decision <> 'create_new_food' OR resolution_method <> 'new_food'));

  IF bad_count > 0 THEN
    RAISE EXCEPTION 'mixed-mode rank policy violation: expected rank 25 resolve_existing and others create_new_food';
  END IF;

  SELECT COUNT(*) INTO bad_count
  FROM wave_decisions_stg
  WHERE priority_rank = 25
    AND candidate_ciqual_code <> '20506';
  ...
  WHERE d.decision = 'create_new_food'
    AND (
      (nf.priority_rank IN (24,26,27) AND nf.category_code <> 'legumineuses')
      OR
      (nf.priority_rank IN (28,29) AND nf.category_code <> 'noix_et_graines')
    );

  IF bad_count > 0 THEN
    RAISE EXCEPTION 'create_new_food category map validation failed (24/26/27 -> legumineuses, 28/29 -> noix_et_graines)';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'measurement food_id must be empty for wave_gos02 mixed flow';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'threshold food_id must be empty for wave_gos02 mixed flow';
  END IF;
END $$;
...
DELETE FROM food_category_memberships AS fcm
USING category_map AS cm
WHERE fcm.food_id = cm.food_id
  AND fcm.source_id = cm.source_id;
...
SET
  ...
  resolved_by = COALESCE(NULLIF(d.reviewed_by, ''), 'wave_gos02_apply'),
  updated_at = now()
FROM wave_decisions_stg AS d
JOIN LATERAL (
  SELECT fer.food_id
...
WHERE d.decision = 'resolve_existing'
  ...
  AND (
    ...
    OR p.resolved_by IS DISTINCT FROM COALESCE(NULLIF(d.reviewed_by, ''), 'wave_gos02_apply')
...
SELECT
  'gos_wave02_apply_summary' AS summary,
