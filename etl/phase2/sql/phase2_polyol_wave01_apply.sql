...
-- Phase 2.7 Polyol Wave 1 execution script (mutating, idempotent)
-- Scope lock: priority_rank IN (30,31,32,33,35,36)
...
DO $$
DECLARE
  expected_ranks INTEGER[] := ARRAY[30,31,32,33,35,36];
  got_ranks INTEGER[];
  bad_count INTEGER;
BEGIN
  ...
  SELECT COUNT(*) INTO bad_count
  FROM wave_decisions_stg
  WHERE (priority_rank = 32 AND (decision <> 'create_new_food' OR resolution_method <> 'new_food'))
     OR (priority_rank <> 32 AND (decision <> 'resolve_existing' OR resolution_method NOT IN ('name_match', 'slug_match', 'manual')));

  IF bad_count > 0 THEN
    RAISE EXCEPTION 'mixed-mode rank policy violation: expected rank 32 create_new_food and others resolve_existing';
  END IF;

  SELECT COUNT(*) INTO bad_count
  FROM wave_decisions_stg
  WHERE (priority_rank = 30 AND candidate_ciqual_code <> '13039')
     OR (priority_rank = 31 AND candidate_ciqual_code <> '13037')
     OR (priority_rank = 33 AND candidate_ciqual_code <> '13043')
     OR (priority_rank = 35 AND candidate_ciqual_code <> '13100')
     OR (priority_rank = 36 AND candidate_ciqual_code <> '13042');
  ...
  WHERE d.decision = 'create_new_food'
    AND (
      (nf.priority_rank = 32 AND nf.category_code <> 'fruits')
      OR
      (nf.priority_rank <> 32)
    );

  IF bad_count > 0 THEN
    RAISE EXCEPTION 'create_new_food category map validation failed (32 -> fruits only)';
  END IF;

  SELECT COUNT(*) INTO bad_count
  FROM wave_measurements_stg
  WHERE fodmap_subtype <> 'sorbitol'
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'measurement food_id must be empty for wave_polyol01 mixed flow';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'threshold food_id must be empty for wave_polyol01 mixed flow';
  END IF;
END $$;
...
SET
  ...
  resolved_by = COALESCE(NULLIF(d.reviewed_by, ''), 'wave_polyol01_apply'),
  updated_at = now()
FROM wave_decisions_stg AS d
JOIN LATERAL (
  SELECT fer.food_id
...
WHERE d.decision = 'resolve_existing'
  ...
  AND (
    ...
    OR p.resolved_by IS DISTINCT FROM COALESCE(NULLIF(d.reviewed_by, ''), 'wave_polyol01_apply')
...
SELECT
  'polyol_wave01_apply_summary' AS summary,
