...
-- Phase 2.7 Polyol Wave 2 execution script (mutating, idempotent)
...
\copy wave_decisions_stg (priority_rank,food_label,variant_label,decision,resolution_method,candidate_food_id,candidate_ciqual_code,new_food_slug,new_food_name_fr,new_food_name_en,new_food_preparation_state,resolution_notes,reviewed_by,reviewed_at) FROM 'etl/phase2/decisions/phase2_polyol_wave02_decisions.csv' WITH (FORMAT csv, HEADER true)
\copy wave_new_foods_stg (priority_rank,new_food_slug,new_food_name_fr,new_food_name_en,new_food_preparation_state,status,custom_ref_value,category_code,notes) FROM 'etl/phase2/data/phase2_polyol_wave02_new_foods.csv' WITH (FORMAT csv, HEADER true)
\copy wave_measurements_stg (priority_rank,food_id,fodmap_subtype,amount_per_100g,comparator,serving_g,amount_per_serving,source_slug,citation_ref,evidence_tier,confidence_score,observed_at,method,notes) FROM 'etl/phase2/data/phase2_polyol_wave02_measurements.csv' WITH (FORMAT csv, HEADER true)
\copy wave_thresholds_stg (priority_rank,food_id,fodmap_subtype,serving_g,low_max_g,moderate_max_g,source_slug,citation_ref,evidence_tier,confidence_score,valid_from,notes) FROM 'etl/phase2/data/phase2_polyol_wave02_thresholds.csv' WITH (FORMAT csv, HEADER true)

DO $$
DECLARE
  expected_ranks INTEGER[] := ARRAY[37,38,42];
  got_ranks INTEGER[];
  bad_count INTEGER;
BEGIN
  ...
  SELECT COUNT(*) INTO bad_count
  FROM wave_decisions_stg
  WHERE (priority_rank = 38 AND (decision <> 'create_new_food' OR resolution_method <> 'new_food'))
     OR (priority_rank <> 38 AND (decision <> 'resolve_existing' OR resolution_method NOT IN ('name_match', 'slug_match', 'manual')));

  IF bad_count > 0 THEN
    RAISE EXCEPTION 'mixed-mode rank policy violation: expected rank 38 create_new_food and others resolve_existing';
  END IF;

  SELECT COUNT(*) INTO bad_count
  FROM wave_decisions_stg
  WHERE (priority_rank = 37 AND candidate_ciqual_code <> '20056')
     OR (priority_rank = 42 AND candidate_ciqual_code <> '20023');
  ...
  WHERE d.decision = 'create_new_food'
    AND (
      (nf.priority_rank = 38 AND nf.category_code <> 'legumes_et_verdures')
      OR
      (nf.priority_rank <> 38)
    );

  IF bad_count > 0 THEN
    RAISE EXCEPTION 'create_new_food category map validation failed (38 -> legumes_et_verdures only)';
  END IF;

  SELECT COUNT(*) INTO bad_count
  FROM wave_measurements_stg
  WHERE fodmap_subtype <> 'mannitol'
     ...
     OR source_slug <> 'yao_2005_polyols'
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'measurement food_id must be empty for wave_polyol02 mixed flow';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'threshold food_id must be empty for wave_polyol02 mixed flow';
  END IF;
END $$;
...
SELECT
  'polyol_wave02_apply_summary' AS summary,
