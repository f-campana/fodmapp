...
WHERE p.priority_rank IN (3,6,7,8,9,10)
...
BEGIN
  ...
  IF v_resolved_links <> 16 THEN
    RAISE EXCEPTION 'resolved links check failed: expected 16, got %', v_resolved_links;
  END IF;

  IF v_unresolved_rows <> 26 THEN
    RAISE EXCEPTION 'unresolved check failed: expected 26, got %', v_unresolved_rows;
  END IF;
  ...
  FROM phase2_priority_foods
  WHERE priority_rank IN (3,6,7,8,9,10);
  ...
  IF COALESCE(v_fructan_resolved, -1) <> 12 THEN
    RAISE EXCEPTION 'fructan gap resolved check failed: expected 12, got %', COALESCE(v_fructan_resolved, -1);
  END IF;

  IF COALESCE(v_fructan_completed, -1) <> 11 THEN
    RAISE EXCEPTION 'fructan gap completed check failed: expected 11, got %', COALESCE(v_fructan_completed, -1);
  END IF;
  ...
  IF v_with_candidates <> 16 THEN
    RAISE EXCEPTION 'candidate pool check failed: expected 16 unresolved-with-candidates, got %', v_with_candidates;
  END IF;

  IF v_without_candidates <> 10 THEN
    RAISE EXCEPTION 'candidate pool check failed: expected 10 unresolved-without-candidates, got %', v_without_candidates;
  END IF;
  ...
  FROM (
    ...
    WHERE ffm.source_record_ref IN (
      'monash_app_garlic_infused_oil_technique_proxy_v1',
      'muir2007_fructan_onion_powder_proxy',
      'muir2007_fructan_spring_onion_bulb',
      'monash_app_spring_onion_green_proxy_v1',
      'muir2007_fructan_shallot_bulb',
      'muir2007_fructan_leek_white_part'
    )
    GROUP BY
      ffm.food_id,
      ffm.fodmap_subtype_id,
...
SELECT
  'PASS' AS phase2_fructan_wave01_checks,
  COUNT(*) FILTER (WHERE priority_rank IN (3,6,7,8,9,10) AND status = 'threshold_set') AS wave_threshold_set_rows,
