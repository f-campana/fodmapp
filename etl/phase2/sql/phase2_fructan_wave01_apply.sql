...
-- Phase 2.5 Wave 1 execution script (mutating, idempotent)
-- Scope lock: priority_rank IN (3,6,7,8,9,10)
...
\copy wave_decisions_stg (priority_rank,food_label,variant_label,decision,resolution_method,candidate_food_id,candidate_ciqual_code,new_food_slug,new_food_name_fr,new_food_name_en,new_food_preparation_state,resolution_notes,reviewed_by,reviewed_at) FROM 'etl/phase2/decisions/phase2_fructan_wave01_decisions.csv' WITH (FORMAT csv, HEADER true)
\copy wave_new_foods_stg (priority_rank,new_food_slug,new_food_name_fr,new_food_name_en,new_food_preparation_state,status,custom_ref_value,category_code,notes) FROM 'etl/phase2/data/phase2_fructan_wave01_new_foods.csv' WITH (FORMAT csv, HEADER true)
\copy wave_measurements_stg (priority_rank,food_id,fodmap_subtype,amount_per_100g,comparator,serving_g,amount_per_serving,source_slug,citation_ref,evidence_tier,confidence_score,observed_at,method,notes) FROM 'etl/phase2/data/phase2_fructan_wave01_measurements.csv' WITH (FORMAT csv, HEADER true)
\copy wave_thresholds_stg (priority_rank,food_id,fodmap_subtype,serving_g,low_max_g,moderate_max_g,source_slug,citation_ref,evidence_tier,confidence_score,valid_from,notes) FROM 'etl/phase2/data/phase2_fructan_wave01_thresholds.csv' WITH (FORMAT csv, HEADER true)

DO $$
DECLARE
  expected_ranks INTEGER[] := ARRAY[3,6,7,8,9,10];
  got_ranks INTEGER[];
  bad_count INTEGER;
BEGIN
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'measurement food_id must be empty for wave01 create_new_food flow';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'threshold food_id must be empty for wave01 create_new_food flow';
  END IF;
END $$;
...
SET
  ...
  resolved_by = COALESCE(NULLIF(d.reviewed_by, ''), 'wave01_apply'),
  updated_at = now()
...
WHERE p.priority_rank = d.priority_rank
  AND (
    ...
    OR p.resolved_by IS DISTINCT FROM COALESCE(NULLIF(d.reviewed_by, ''), 'wave01_apply')
...
SELECT
  'fructan_wave01_apply_summary' AS summary,
