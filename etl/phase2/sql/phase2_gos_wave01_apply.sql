...
-- Phase 2.6 GOS Wave 1 execution script (mutating, idempotent)
...
\copy wave_decisions_stg (priority_rank,food_label,variant_label,decision,resolution_method,candidate_food_id,candidate_ciqual_code,new_food_slug,new_food_name_fr,new_food_name_en,new_food_preparation_state,resolution_notes,reviewed_by,reviewed_at) FROM 'etl/phase2/decisions/phase2_gos_wave01_decisions.csv' WITH (FORMAT csv, HEADER true)
\copy wave_new_foods_stg (priority_rank,new_food_slug,new_food_name_fr,new_food_name_en,new_food_preparation_state,status,custom_ref_value,category_code,notes) FROM 'etl/phase2/data/phase2_gos_wave01_new_foods.csv' WITH (FORMAT csv, HEADER true)
\copy wave_measurements_stg (priority_rank,food_id,fodmap_subtype,amount_per_100g,comparator,serving_g,amount_per_serving,source_slug,citation_ref,evidence_tier,confidence_score,observed_at,method,notes) FROM 'etl/phase2/data/phase2_gos_wave01_measurements.csv' WITH (FORMAT csv, HEADER true)
\copy wave_thresholds_stg (priority_rank,food_id,fodmap_subtype,serving_g,low_max_g,moderate_max_g,source_slug,citation_ref,evidence_tier,confidence_score,valid_from,notes) FROM 'etl/phase2/data/phase2_gos_wave01_thresholds.csv' WITH (FORMAT csv, HEADER true)

DO $$
DECLARE
  expected_ranks INTEGER[] := ARRAY[18,19,20,21,22,23];
  got_ranks INTEGER[];
  bad_count INTEGER;
BEGIN
  ...
  WHERE decision NOT IN ('resolve_existing', 'create_new_food');

  IF bad_count > 0 THEN
    RAISE EXCEPTION 'decision policy violation: unsupported decision value in wave_decisions_stg';
  END IF;

  SELECT COUNT(*) INTO bad_count
  FROM wave_decisions_stg
  WHERE (decision = 'resolve_existing' AND resolution_method NOT IN ('name_match', 'slug_match', 'manual'))
     OR (decision = 'create_new_food' AND resolution_method <> 'new_food');

  IF bad_count > 0 THEN
    RAISE EXCEPTION 'resolution_method policy violation in wave_decisions_stg';
  END IF;

  SELECT COUNT(*) INTO bad_count
  FROM wave_decisions_stg
  WHERE (priority_rank = 21 AND (decision <> 'create_new_food' OR resolution_method <> 'new_food'))
     OR (priority_rank <> 21 AND (decision <> 'resolve_existing' OR resolution_method NOT IN ('name_match', 'slug_match', 'manual')));

  IF bad_count > 0 THEN
    RAISE EXCEPTION 'mixed-mode rank policy violation: expected rank 21 create_new_food and others resolve_existing';
  END IF;

  SELECT COUNT(*) INTO bad_count
  FROM wave_decisions_stg
  WHERE (priority_rank = 18 AND candidate_ciqual_code <> '20532')
     OR (priority_rank = 19 AND candidate_ciqual_code <> '20587')
     OR (priority_rank = 20 AND candidate_ciqual_code <> '20589')
     OR (priority_rank = 22 AND candidate_ciqual_code <> '20502')
     OR (priority_rank = 23 AND candidate_ciqual_code <> '20503');

  IF bad_count > 0 THEN
    RAISE EXCEPTION 'locked candidate CIQUAL mapping mismatch in wave_decisions_stg';
  END IF;
  ...
  JOIN wave_decisions_stg AS d
    ON d.priority_rank = nf.priority_rank
  WHERE d.decision = 'create_new_food'
    AND (
      nf.new_food_slug IS NULL
      OR nf.new_food_slug = ''
      OR nf.new_food_slug NOT LIKE 'phase2-%'
      OR nf.new_food_name_fr IS NULL
      OR nf.new_food_name_fr = ''
      OR nf.status <> 'draft'
      OR nf.custom_ref_value <> ('phase2_pass3:' || nf.priority_rank::TEXT)
      OR nf.category_code IS NULL
      OR nf.category_code = ''
      OR nf.new_food_preparation_state NOT IN ('raw','cooked','processed','fermented','rehydrated','unknown')
    );

  IF bad_count > 0 THEN
    RAISE EXCEPTION 'create_new_food scaffold validation failed';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'unknown category_code for create_new_food rows';
  END IF;
  ...
  JOIN wave_decisions_stg AS d
    ON d.priority_rank = nf.priority_rank
  WHERE d.decision = 'create_new_food'
    AND d.priority_rank = 21
    AND nf.category_code <> 'legumineuses';

  IF bad_count > 0 THEN
    RAISE EXCEPTION 'rank 21 category check failed: expected category_code=legumineuses';
  END IF;

  SELECT COUNT(*) INTO bad_count
  FROM wave_measurements_stg
  WHERE fodmap_subtype <> 'gos'
  ...
  IF bad_count <> 1 THEN
    RAISE EXCEPTION 'required source_slug missing or duplicated: internal_rules_v1';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'measurement food_id must be empty for wave_gos01 create_new_food flow';
  END IF;
  ...
  IF bad_count > 0 THEN
    RAISE EXCEPTION 'threshold food_id must be empty for wave_gos01 create_new_food flow';
  END IF;
END $$;
...
missing_foods AS (
  ...
  JOIN wave_decisions_stg AS d
    ON d.priority_rank = nf.priority_rank
   AND d.decision = 'create_new_food'
...
SET
  ...
  resolved_by = COALESCE(NULLIF(d.reviewed_by, ''), 'wave_gos01_apply'),
  updated_at = now()
FROM wave_decisions_stg AS d
JOIN LATERAL (
  SELECT fer.food_id
...
WHERE d.decision = 'resolve_existing'
  ...
  AND (
    ...
    OR p.resolved_by IS DISTINCT FROM COALESCE(NULLIF(d.reviewed_by, ''), 'wave_gos01_apply')
...
SELECT
  'gos_wave01_apply_summary' AS summary,
