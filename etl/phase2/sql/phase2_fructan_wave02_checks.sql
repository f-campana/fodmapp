...
WHERE p.priority_rank IN (11,12,13,16,17)
...
DECLARE
  ...
  v_fructan_resolved INTEGER;
  v_fructan_completed INTEGER;
  v_fructan_pending INTEGER;
...
BEGIN
  ...
  IF v_resolved_links <> 21 THEN
    RAISE EXCEPTION 'resolved links check failed: expected 21, got %', v_resolved_links;
  END IF;

  IF v_unresolved_rows <> 21 THEN
    RAISE EXCEPTION 'unresolved check failed: expected 21, got %', v_unresolved_rows;
  END IF;
  ...
  FROM phase2_priority_foods
  WHERE priority_rank IN (11,12,13,16,17);

  IF v_wave_resolved <> 5 THEN
    RAISE EXCEPTION 'wave resolution check failed: expected 5 resolved, got %', v_wave_resolved;
  END IF;

  IF v_wave_threshold_set <> 5 THEN
    RAISE EXCEPTION 'wave status check failed: expected 5 threshold_set, got %', v_wave_threshold_set;
  END IF;
  ...
  IF v_wave_measurements <> 5 THEN
    RAISE EXCEPTION 'wave measurement coverage failed: expected 5, got %', v_wave_measurements;
  END IF;
  ...
  IF v_wave_thresholds <> 5 THEN
    RAISE EXCEPTION 'wave threshold coverage failed: expected 5, got %', v_wave_thresholds;
  END IF;
  ...
  SELECT
    ...
    completed_rows,
    pending_measurement_rows
  INTO v_fructan_resolved, v_fructan_completed, v_fructan_pending
  FROM v_phase2_gap_completion
  WHERE gap_bucket = 'fructan_dominant'
    AND target_subtype = 'fructan';

  IF COALESCE(v_fructan_resolved, -1) <> 17 THEN
    RAISE EXCEPTION 'fructan gap resolved check failed: expected 17, got %', COALESCE(v_fructan_resolved, -1);
  END IF;

  IF COALESCE(v_fructan_completed, -1) <> 16 THEN
    RAISE EXCEPTION 'fructan gap completed check failed: expected 16, got %', COALESCE(v_fructan_completed, -1);
  END IF;

  IF COALESCE(v_fructan_pending, -1) <> 1 THEN
    RAISE EXCEPTION 'fructan gap pending check failed: expected 1, got %', COALESCE(v_fructan_pending, -1);
  END IF;
  ...
  IF v_with_candidates <> 15 THEN
    RAISE EXCEPTION 'candidate pool check failed: expected 15 unresolved-with-candidates, got %', v_with_candidates;
  END IF;
  ...
  FROM (
    ...
    WHERE ffm.source_record_ref IN (
      'biesiekierski2011_fructan_wheat_t55_proxy',
      'biesiekierski2011_fructan_wheat_t65_proxy',
      'biesiekierski2011_fructan_wheat_t80_proxy',
      'muir2007_fructan_artichoke_heart_proxy',
      'muir2007_fructan_chicory_root_proxy'
    )
    GROUP BY
      ffm.food_id,
      ffm.fodmap_subtype_id,
...
SELECT
  'PASS' AS phase2_fructan_wave02_checks,
  COUNT(*) FILTER (WHERE priority_rank IN (11,12,13,16,17) AND status = 'threshold_set') AS wave_threshold_set_rows,
