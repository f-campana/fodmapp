...
-- Phase 2.6 GOS Wave 2 hard checks
...
WHERE p.priority_rank IN (24,25,26,27,28,29)
...
BEGIN
  ...
  IF v_resolved_links <> 33 THEN
    RAISE EXCEPTION 'resolved links check failed: expected 33, got %', v_resolved_links;
  END IF;

  IF v_unresolved_rows <> 9 THEN
    RAISE EXCEPTION 'unresolved check failed: expected 9, got %', v_unresolved_rows;
  END IF;
  ...
  FROM phase2_priority_foods
  WHERE priority_rank IN (24,25,26,27,28,29);
  ...
  IF COALESCE(v_gos_resolved, -1) <> 12 THEN
    RAISE EXCEPTION 'gos gap resolved check failed: expected 12, got %', COALESCE(v_gos_resolved, -1);
  END IF;

  IF COALESCE(v_gos_completed, -1) <> 12 THEN
    RAISE EXCEPTION 'gos gap completed check failed: expected 12, got %', COALESCE(v_gos_completed, -1);
  END IF;

  IF COALESCE(v_gos_unresolved, -1) <> 0 THEN
    RAISE EXCEPTION 'gos gap unresolved check failed: expected 0, got %', COALESCE(v_gos_unresolved, -1);
  END IF;
  ...
  IF v_with_candidates <> 6 THEN
    RAISE EXCEPTION 'candidate pool check failed: expected 6 unresolved-with-candidates, got %', v_with_candidates;
  END IF;

  IF v_without_candidates <> 3 THEN
    RAISE EXCEPTION 'candidate pool check failed: expected 3 unresolved-without-candidates, got %', v_without_candidates;
  END IF;
  ...
  FROM (
    ...
    WHERE ffm.source_record_ref IN (
      'dysseler_hoffem_gos_black_bean_cooked_proxy',
      'dysseler_hoffem_gos_split_pea_cooked_proxy',
      'dysseler_hoffem_gos_edamame_cooked_proxy',
      'dysseler_hoffem_gos_soybean_whole_cooked_proxy',
      'dysseler_hoffem_gos_cashew_raw_proxy',
      'dysseler_hoffem_gos_pistachio_raw_proxy'
    )
    GROUP BY
      ffm.food_id,
      ffm.fodmap_subtype_id,
...
SELECT
  'PASS' AS phase2_gos_wave02_checks,
  COUNT(*) FILTER (WHERE priority_rank IN (24,25,26,27,28,29) AND status = 'threshold_set') AS wave_threshold_set_rows,
...
WITH latest_measurements AS (
  ...
  JOIN sources AS s
   ...
   AND s.source_slug = 'dysseler_hoffem_gos'
  WHERE p.priority_rank IN (24,25,26,27,28,29)
