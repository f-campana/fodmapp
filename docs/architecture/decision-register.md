...
| ADR-020  | Serve Phase 3 rollups/subtypes/swaps through versioned published read models before hosted activation                 | In progress | Active in `codex/publish-boundary-dbmate-bridge`; ports immutable `publish_id` releases and `api_*_current` views onto the merged dbmate lane so API traffic no longer depends on drop/recreate snapshots |
...
- Current implementation track: `codex/publish-boundary-dbmate-bridge`.
