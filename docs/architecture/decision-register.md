...
| ADR-008  | Use `dbmate` for SQL-first migration workflow                                                                          | Implemented | Baseline migration lane, repo commands, and smoke validation landed on `main` via PR #287.                                                                                                               |
| ADR-009  | Neon branch policy (`main` prod, `staging`, `pr-*`)                                                                    | Planned     | `dbmate`, published reads, and manual promote are now on `main`; first hosted activation remains blocked on the separate initial persistent bootstrap track.                                             |
...
| ADR-020  | Serve Phase 3 rollups/subtypes/swaps through versioned published read models before hosted activation                  | Implemented | Landed on `main` via PR #288 with immutable `publish_id` releases and `api_*_current` views for rollups, subtypes, and swaps.                                                                            |
| ADR-021  | Use Koyeb + Neon for the first hosted API environment, with hosting deferred until initial persistent bootstrap exists | Accepted    | Koyeb bootstrap assets and CI container smoke are tracked separately from hosted activation; `api.fodmapp.fr` remains planned, not live, until the later bootstrap track exists.                         |
...
- Current state: proactive bootstrap landed on `main` via PR #287.
...
- Current state: landed on `main` via PR #288.

4. When to activate the first hosted API:

- Koyeb + Neon is the accepted first hosted path.
- `dbmate`, published reads, and `phase3:promote` are already on `main`.
- Activation remains blocked until the repo has a dedicated initial persistent data bootstrap path for Neon.
