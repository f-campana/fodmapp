...
| `/Users/fabiencampana/Documents/Fodmap-worktrees/ui-state-controls-batch-f`         | `codex/ui-state-controls-batch-f`         | active             | Batch F state controls (`alert-dialog`, `slider`, `switch`, `toggle`, `toggle-group`) | In progress; nested stories/docs and contract updates under review. |
